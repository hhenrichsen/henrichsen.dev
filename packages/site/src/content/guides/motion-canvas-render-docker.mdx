---
title: "Rendering Motion Canvas in Docker"
date: 2024-08-27T17:00:00-06:00
draft: false
type: "guide"
growth: "bud"
static: "motion-canvas-render-docker"
tags:
  - typescript
  - motion-canvas
  - puppeteer
...
```ts title="src/app/rendering.test.ts"
...
I started with a simple dockerfile. I'm using `alpine-chrome` so that WebGL
...
This one _works_, but isn't very good because each time you run it, it will
...
`devops/docker-compose.yml` file that looks like this:

```yaml title="devops/docker-compose.yml"
services:
  render: &render
    build:
      context: ..
...
Discord, either on [my personal server](https://discord.gg/hHxXWy7FRQ) or the
[Motion Canvas server](https://chat.motioncanvas.io/).
